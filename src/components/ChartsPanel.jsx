...
const ChartsPanel = ({ 
  ...
  kpiTrends = [],
  selectedTimeRange,
  focusMode = 'all' // all, environmental, production, analytics
}) => {
  ...
  const prepareCombinedData = () => {
    // Combina dati ambientali e KPI per analisi correlazioni
  ...
  const getChartTypesByFocus = () => {
    switch (focusMode) {
      case 'environmental':
        return chartTypes.filter(t => ['overview', 'environmental', 'correlation'].includes(t.id));
      case 'production':
        return chartTypes.filter(t => ['overview', 'performance', 'correlation'].includes(t.id));
      case 'analytics':
        return chartTypes;
      default:
        return chartTypes;
          ...
          {payload.map((entry, index) => (
            <p key={index} style={{ color: entry.color }} className="text-sm">
              ...
              {entry.name.includes('Temperatura') ? '°C' :
               ...
               entry.name.includes('Vento') ? ' km/h' :
  ...
  const renderOverviewChart = () => (
    <div className="space-y-6">
      {/* Grafico combinato ambiente-performance */}
  ...
  const renderEnvironmentalChart = () => (
    <div className="chart-16x9">
      <h4 className="text-md font-semibold mb-4">Condizioni Ambientali nel Tempo</h4>
      <ResponsiveContainer width="100%" height="100%">
  ...
  const renderCorrelationChart = () => (
    <div className="chart-16x9">
      <h4 className="text-md font-semibold mb-4">Correlazione Temperatura-Efficienza</h4>
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={prepareCombinedData()}>
          <CartesianGrid 
            strokeDasharray="3 3" 
          ...
          <div className="text-center">
            ...
            <p className="text-lg font-semibold">
              {environmentalTrends.length > 0 
                ? format(new Date(environmentalTrends[environmentalTrends.length - 1].timestamp), 'HH:mm')
                : '--:--'
              }
            </p>
          </div>
