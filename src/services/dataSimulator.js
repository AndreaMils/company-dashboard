/**
 * Simulatore di dati ambientali e di produzione per il settore agricolo
 * Simula un'azienda agricola che coltiva grano, mais, pomodori e olive
 */

import { format, subDays } from 'date-fns';

// Configurazione delle colture
const CROPS = {
  wheat: {
    name: 'Grano',
    area: 25, // ettari
    plantingMonth: 10, // ottobre
    harvestMonth: 6, // giugno
    avgYieldPerHa: 5.5, // tonnellate per ettaro
    pricePerTon: 250, // euro per tonnellata
    growthDays: 240,
    optimalTemp: { min: 15, max: 25 },
    optimalHumidity: { min: 60, max: 80 },
    waterNeed: 400 // mm di acqua durante il ciclo
  },
  corn: {
    name: 'Mais',
    area: 20,
    plantingMonth: 4, // aprile
    harvestMonth: 9, // settembre
    avgYieldPerHa: 8.5,
    pricePerTon: 220,
    growthDays: 150,
  ...
  tomatoes: {
    name: 'Pomodori',
    area: 5,
    plantingMonth: 3, // marzo
    harvestMonth: 8, // agosto
    avgYieldPerHa: 45,
    pricePerTon: 400,
    growthDays: 120,
  ...
  olives: {
    name: 'Olive',
    area: 15,
    plantingMonth: 0, // perenne
    harvestMonth: 10, // ottobre
    avgYieldPerHa: 3.5,
    pricePerTon: 800,
    growthDays: 365,
...
// Generatori di dati con distribuzione statistica realistica
class DataSimulator {
  constructor() {
  ...
  // Generatore di numeri con distribuzione normale (Box-Muller)
  ...
  // Simulazione temperatura con variazioni stagionali e giornaliere
  generateTemperature() {
  ...
  // Simulazione umidità correlata alla temperatura
  ...
  // Simulazione precipitazioni con eventi casuali
  generateRainfall() {
    ...
    if (Math.random() < seasonalProbability[this.season]) {
      // Evento di pioggia - distribuzione esponenziale
      return Math.min(50, -Math.log(Math.random()) * 8);
    }
    return 0;
  }

  // Simulazione velocità del vento
  generateWindSpeed() {
    const baseWind = 10;
    const seasonalWind = {
      spring: 3,
  ...
  // Calcolo indice UV basato su stagione e ora
  generateUVIndex() {
  ...
  // Calcolo crescita delle colture basato su condizioni ambientali
  calculateCropGrowth(cropType, environmentalData) {
    ...
    // Disponibilità idrica (accumulo precipitazioni)
    const waterAvailability = Math.min(1, rainfall / 5); // 5mm = buona disponibilità

    // Crescita giornaliera normale
    const baseGrowth = 100 / crop.growthDays; // percentuale al giorno
    
    // Applicazione dei fattori di stress
  ...
  calculateStressFactor(value, optimalMin, optimalMax) {
    if (value >= optimalMin && value <= optimalMax) {
      return 1.0; // Condizioni ottimali
    }
  ...
  // Simulazione produzione con variabilità realistica
  ...
  calculateRevenue(cropType, production) {
    const crop = CROPS[cropType];
    const marketVariability = this.normalRandom(1, 0.1); // ±10% variabilità prezzo
  ...
  // Generazione storico dati per grafici
  generateHistoricalData(days = 30) {
    ...
    for (let i = days; i >= 0; i--) {
      ...
      // Aggiorna crescita per il giorno successivo
